...
// get a list of all substrings
// filter through substrings
// return substrings are that longer than 1 chr
// and if substring equals itself reversed
...
function palindromes(string) {
  return substrings(string).filter(sub => {
    if (sub.length > 1 && sub.split("").reverse().join("") === sub) {
      return sub;
    }
  });
}
