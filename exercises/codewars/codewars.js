...
/*
How many files can you store in the drive. The files must be saved in the order they appear
...
check how many files can be stored in drive,
array contains the files and their sizes
integer is drive size
...
edge cases:
  ...
  always array input
  can be empty array input => return 0
  if integer (drive size) is zero return 0
  will input array always contain all elements or will it be sparse
    => use filter to remove empty elements
  should I return errors or message if input is invalid (like non array)

test cases:

  ([1, 2], 0) // 0
  ([5, 10, 15, 20, 3, 6, 9], 51) // 4
  ([], 0) // 0
  (1, 5) // "invalid"
  ([1, 2], -5) // 0
...
algo:
  ...
  check if input array is an array, else return "invalid"
  check if input array is empty, if yes return 0
  check if input integer is < 1, if yes return 0
  fix sparsed array
  create a count variable = 0
  create a for loop with idx = 0
    for each element in array do this:
      if drive size - curr element is >= 0
        subtract curr element from drive size
        add 1 to count
      else
        return count;
*/

// function driveSize(files, size) {
//   if (!Array.isArray(files)) {
//     return "invalid";
//   }

//   files = files.filter(num => num);

//   if (files.length === 0 || size < 1) {
//     return 0;
//   }

//   return updateDrive(files, size);
// }

// function updateDrive(files, size) {
//   let count = 0;
...
// console.log(driveSize([4, 4, 4, 3, 3], 12)); // 3
// console.log(driveSize([4, 4, 4, 3, 3], 11)); // 2
// console.log(driveSize([], 1)); // 0
// console.log(driveSize([1, 2], 0)); // 0
// console.log(driveSize([1, 2], 0)); // 0
// console.log(driveSize([5, 10, 15, 20, 3, 6, 9], 51)); // 4
// console.log(driveSize([], 0)); // 0
// console.log(driveSize(1, 5)); // "invalid"
// console.log(driveSize([1, 2], -5)) // 0
// console.log(driveSize([1, 2, 3, , 4], 5)) // 2
// console.log(driveSize([, , , ,], 5)) // 0
